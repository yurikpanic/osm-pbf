    ...
    (loop for k from 1 to (+ 2 (- max-children (* 2 min-children)))
         do (let* ((first-size (+ k (1- min-children))))
              (dolist (bb-cur bb-sorted)
                (let ((first-bbox (calc-group-bb bb 0 (1- first-size)))
                      (second-bbox (calc-group-bb bb first-size (1- (rnode-size node)))))
                  (format t "~A: ~A ~A~%" first-size first-bbox second-bbox)
                  ))))))
